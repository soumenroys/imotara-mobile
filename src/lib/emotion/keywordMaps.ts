...
export const ROMAN_MR_LANG_HINT_REGEX =
    /\b(kay|kaay|kasa|kashi|kase|bara|baray|thik|thik aahe|aata|atta|aaj|udya|mi|mee|tu|tumhi|mala|tula|tyala|aamhi|aapan|ahe|aahe|navta|navte|ghari|baher|ithe|tithe|kaay chalay|kay chalalay|sang|bol|yete|jaato|khaala|paani|zhala|zala|chhan|khup|ekta|vatat|vatte|aajun|baray nahi|theek nahi|manat|dukhat|rahat nahi|vallagche|vallagtech|dhur na|dhur|nako|naako|yaar re|jevla|jevlas|karu|karuyat|saang|baghtoy|baghtye|aavdta|aavdtay)\b/i;
...
// Hindi confusion / mental overload (renamed + updated from web)
...
export const CONFUSED_HI_REGEX =
    /(समझ नहीं|समझ नही|कन्फ्यूज|भूल गया|भूल गयी|क्या करूँ|क्या करु|क्या करूं|क्या करूँ|\bsamajh\s+nahi\s+aa\s+raha\b|\bsamajh\s+nahi\s+aa\s+rahi\b|\bsamajh\s+nahi\s+aa\s+raha\s+hai\b|\bsamajh\s+nahi\s+aa\s+rahi\s+hai\b|\bkya\s+karu\b|\bkya\s+karoon\b|\bkya\s+karu[nm]?\b)/i;
...
export const GU_CONFUSED_REGEX =
    /(સમજાતું નથી|માથું ચકરાય છે|શું કરવું|ગૂંચવણ|ગૂંચવાઈ ગયો|ગૂંચવાઈ ગઈ|\bsamjhay na\b|\bsamjatu nathi\b|\bshu karvu\b|\bmathu chakray\b|\bgoonchan\b|\bgumchdvayu\b|\bkya karu\b|\bsamjhay nathi\b)/i;

// --------------------------------------------------
// Telugu emotion regexes
//
...
export const KN_CONFUSED_REGEX =
    /(ತಿಳಿಯುತ್ತಿಲ್ಲ|ಅರ್ಥ ಆಗುತ್ತಿಲ್ಲ|ಏನು ಮಾಡಬೇಕು|ತಲೆ ಗೊಂದಲ|ಮನಸ್ಸು ಗೊಂದಲ|\barthu aaguvudilla\b|\bsamje kuda illa\b|\benu maadabeku\b|\btala gondala\b|\bmanasu gondala\b|\btumba confused\b|\byana maadali\b)/i;

// --------------------------------------------------
// Malayalam emotion regexes
// --------------------------------------------------
...
// Tightened for UX-38 before wiring into isSadText. The bare hiragana うつ
// matched inside ordinary words — うつくしい ("beautiful"), 写真をうつす — and
// 一人 is everyday Japanese for "one person", so 一人暮らし ("living alone",
// a neutral fact) read as sadness. Measured 2026-09-05: 4 of 6 neutral
// sentences were flagged. Replaced with 一人ぼっち, which only means lonely.
...
// Tightened for UX-38 before wiring into isSadText. Bare "allein" is ordinary
// German for "alone" and matched "Ich gehe allein einkaufen" — going shopping
// by yourself. The phrases kept are the ones that carry the feeling.
...
// --------------------------------------------------
// Crisis detection
// Covers explicit suicidal ideation, self-harm, sexual assault/danger — across
// all 22 Imotara-supported languages. Synced from imotaraapp's copy
// (2026-08-14) — previously stale at 13 languages; keep in sync manually,
...
export function debugDetectEmotion(text: string): DebugEmotion | null {
    ...
    if (
        BN_STRESS_REGEX.test(input) ||
        HI_STRESS_REGEX.test(input) ||
        TA_STRESS_REGEX.test(input)
    )
        return "stressed";
...
const SAD_REGEXES = [
  ...
  // UX-38. These four were defined and deliberately left unwired until their
  // patterns had been looked at. Arabic and Hebrew measured clean on every
  // neutral sentence tried and went in as they were; Japanese and German
  // needed the tightening noted above first.
...
/**
 * Stress/anxiety, in every Indic language the app defines a pattern for.
 *
 * Written out by hand in five places — ChatScreen twice, HistoryScreen,
 * aiClient twice — and one of the five had quietly lost Odia, so the same
 * Odia sentence was read as stress on the history screen and as nothing at
 * all in the chat mood hint.
 *
 * English deliberately stays at the call site. The five callers do NOT agree
 * on their English wording (one has "tension", another "overwhelmed", and
 * aiClient checks no English at all), so folding one English list in here
 * would silently change four call sites while fixing one. Adding a LANGUAGE
